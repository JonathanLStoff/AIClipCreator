        ...
        try:
            ...
            for _ in tqdm(range(random_time), desc="Waiting for video to play..."):
                ...
                if should_move_mouse < 80:
                    ...
                    for step in range(1, steps + 1):
                        ...
                        move_x = max(
                            0, min(random_x, window_width - 2)
                        )  # Even more conservative boundary
            ...
            element = WebDriverWait(driver, 10).until(
                EC.presence_of_element_located(
                    (By.CSS_SELECTOR, 'h1[data-e2e="video-desc"]')
                )
            )
            desc = element.text
                ...
                try:
                    share_button = WebDriverWait(driver, 10).until(
                        EC.element_to_be_clickable(
                            (
                                By.XPATH,
                                (
                                    "//button[contains(@aria-label, 'Share video') and"
                                    " contains(@aria-label, 'shares')]"
                                ),
                            )
                        )
                    )
                    ...
                    link_button = WebDriverWait(driver, 10).until(
                        EC.element_to_be_clickable(
                            (
                                By.CSS_SELECTOR,
                                "button.TUXButton.TUXButton--default.TUXButton--medium.TUXButton--secondary",
                            )
                        )
                    )
                    ...
                    LOGGER.error(
                        "Failed to click share button: %s", traceback.format_exc()
                    )
            ...
            if "reddit" in desc.lower():
                try:
                    ...
                    LOGGER.error(
                        "Failed to click like button: %s", traceback.format_exc()
                    )
        ...
        try:
            action_button = WebDriverWait(driver, 10).until(
                EC.element_to_be_clickable(
                    (
                        By.CSS_SELECTOR,
                        "button.TUXButton--capsule.TUXButton--medium.TUXButton--secondary.action-item",
                    )
                )
            )
            action_button.click()
