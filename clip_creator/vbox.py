...
def start_android_vbox(vbox_vm_name, android_image_path, mode="screen"):
    ...
    try:
        ...
        if f'"{vbox_vm_name}"' in existing_vms.stdout:
            print(
                f"VM '{vbox_vm_name}' already exists. Skipping creation and"
                " configuration."
            )
        else:
            subprocess.run(
        ...
        vm_status_process = subprocess.run(
            check_vm_status_command, capture_output=True, text=True, check=True
        )
        ...
        if "State: running" in vm_status_output:
            ...
            raise RuntimeError(
                f"VM '{vbox_vm_name}' is already running. Please stop it first."
            )
        elif (
            "State: powered off" not in vm_status_output
            and "State: saved" not in vm_status_output
        ):
            raise RuntimeError(
                f"VM '{vbox_vm_name}' is in an unexpected state: {vm_status_output}"
            )

        # 2. Start the VM with the chosen mode (headless or screen)
        print(f"Starting VM '{vbox_vm_name}' in {mode} mode...")
        subprocess.Popen(
            ["VBoxManage", "startvm", vbox_vm_name, "--type", start_type],
