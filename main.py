...
def main():
    ...
    try:
        ...
        for video in videos:
            ...
            try:
                LOGGER.info(
                    "getting transcript for video id: %s", video["id"]["videoId"]
                )
        ...
        for id, script in formated_transcripts.items():
            try:
                ...
                LOGGER.error(
                    "Error getting reddit comments: %s, for %s",
                    traceback.format_exc(),
                    id,
                )
                errors_lot[id][
        ...
        for id, script in formated_transcripts.items():
            try:
                ...
                LOGGER.error(
                    "Error getting youtube comments: %s, for %s",
                    traceback.format_exc(),
                    id,
                )
                errors_lot[id][
        ...
        for id, script in raw_transcripts.items():
            try:
                ...
                LOGGER.error(
                    "Error adding data: %s, for %s", traceback.format_exc(), id
                )
                errors_lot[id][
                    ERRORS_TYPES[1]
            ...
            try:
                if timestamps[id]:  # Add more checks
                    if "disabled" != script:
                        ...
                        LOGGER.info(
                            "Transcript is disabled, add download and transcribe"
                            " function here"
                        )
                        clips[id] = None
                else:
                ...
                LOGGER.error(
                    "Error finding clips: %s, for %s", traceback.format_exc(), id
                )
                errors_lot[id][
                    ERRORS_TYPES[1]
        ...
        for id, clip in clips.items():
            if clip:
                if (
                    not find_clip(id, int(float(clip[0]["start"])))
                ) or args.inputvideoid:
                    clips_chunks[id] = {
        ...
        for id, clip in clips.items():
            if clip and clips_chunks[id]:
                if ERRORS_TYPES[2] in errors_lot.get(id, {}).keys():
                    continue
                try:
                    LOGGER.info(
                        "FOLDERS: %s %s",
        ...
        for id, clip in clips.items():
            if clip:
                LOGGER.info("Clip: %s", id)
                number_clips += 1
        if number_clips == 0:
            LOGGER.info("No clips to post")
            allsched = []
        else:
            allsched = get_timestamps(number_clips)
        allsched_idx = 0
        for id, clip in clips.items():
            if ERRORS_TYPES[2] in errors_lot[id].keys():
                continue
        ...
        for id, clipy in clips.items():
            try:
                ...
                LOGGER.error(
                    "Error posting to social media: %s, for %s",
                    traceback.format_exc(),
                    id,
                )
                errors_lot[id][
        ...
        LOGGER.error("Errors: %s", errors_lot)
